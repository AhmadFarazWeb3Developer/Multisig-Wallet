...
List of addresses that will control the Safe.
These are the initial owners set at creation time.

uint256 _threshold:
The minimum number of owner signatures required to execute a transaction.
Example: if there are 3 owners and threshold = 2 → any 2 must sign.

address to:
An optional contract to call right after setup.
Often used to enable modules, configure the Safe, or run custom logic during initialization.
If not needed, just set to address(0).

bytes calldata data:
Calldata for the to contract.
Executed via delegatecall inside the Safe.
Empty ("") if you don’t want to call anything.

address fallbackHandler:
Contract that will handle calls to unknown functions (like ERC-1271 signature validation, ERC-165 interface detection, etc.).
If you don’t need advanced features, set address(0).

address paymentToken:
Token to pay deployment fees in (can be any ERC-20).
If set to address(0), payment will be in ETH.

uint256 payment:
Amount to pay the deployer or relayer for creating the Safe.
If 0, no payment is made.

address payable paymentReceiver:
Address that receives the payment.
Could be a relayer, factory, or service provider.
If payment = 0, this field is ignored.
    */
